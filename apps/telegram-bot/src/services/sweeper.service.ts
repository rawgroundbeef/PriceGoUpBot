...
export class SweeperService {
  ...
  async sweepAllPendingPayments(): Promise<{
    ...
    try {
      ...
      console.log(`🔍 Checking ${ordersToCheck.length} orders for payments to sweep`);
      ...
      console.log(`✅ Sweep completed: ${result.swept}/${result.processed} orders swept`);
      return result;
    } catch (error) {
  ...
  async sweepOrderPayment(orderId: string): Promise<boolean> {
    ...
    try {
      ...
      if (order.payment_address && order.payment_address !== 'TEMP') {
        ...
        try {
          ...
          if (derivedKeypair.publicKey.toString() === order.payment_address) {
            ...
            console.log(`🔑 Using HKDF-derived keypair for order ${orderId.substring(0, 8)}`);
          } else {
            console.log(`⚠️ Order ${orderId.substring(0, 8)} uses legacy address, cannot derive private key`);
            return false;
          }
        } catch {
          console.log(`⚠️ Order ${orderId.substring(0, 8)} cannot derive keypair`);
          return false;
        }
      } else {
      ...
      console.log(`💰 Order ${orderId.substring(0, 8)}: ${balance / LAMPORTS_PER_SOL} SOL in payment address`);

      if (balance < (minSweepLamports || 1000000)) {
        console.log(`⚠️ Balance too low to sweep (${balance} < ${minSweepLamports})`);
        return false;
      }
      ...
      if (balance < expectedLamports * 0.99) { // 1% tolerance
        console.log(`⚠️ Payment insufficient: ${balance} < ${expectedLamports} lamports`);
        return false;
      }
      ...
      console.log(`💸 Splitting ${balance / LAMPORTS_PER_SOL} SOL: ${serviceFeeAmount / LAMPORTS_PER_SOL} fees, ${opsAmount / LAMPORTS_PER_SOL} ops`);
      ...
      console.log(`✅ Swept order ${orderId.substring(0, 8)}: fees=${feesSig.substring(0, 8)}..., ops=${opsSig.substring(0, 8)}...`);
      return true;
  ...
  private async sendToTreasury(
    ...
    console.log(`💸 Sent ${lamports / LAMPORTS_PER_SOL} SOL to ${purpose} treasury: ${signature.substring(0, 8)}...`);
    return signature;
  }
}
